...
import { calculateBreakEven, calculateDvcTripCost, calculatePackageTripCost, findBreakEvenYear, usdToGbp } from "../lib/calculations";
...
export function BreakEvenChart({ state, years }: Props) {
  ...
  const points = calculateBreakEven(purchaseCostGbp, annualDuesGbp, packageAnnualGbp, years);
  ...
  return (
    <section>
      ...
      <p className="muted">
        Cumulative cost of buying DVC (one-time purchase + annual dues, both converted to GBP)
        vs. always booking the package holiday instead, assuming this exact trip every year.{" "}
        {breakEvenYear
